...
该版本通过 GeoGebra 官方 CDN 加载运行库，同时从本地 `assets` 目录读取两个 `.ggb` 实验文件。使用时需要联网。
...
```bash
cd /Users/tyousann/Desktop/备课/20260623赛课/qayzzxd.github.io
```
...
库仑扭秤使用与 GGB 文件匹配的三维运行库：

```text
https://www.geogebra.org/apps/5.4.920.0/web3d/
```

`assets/1.ggb` 和 `assets/2.ggb` 始终从本地读取，不会上传到 GeoGebra。
...
因此 GeoGebra 实验无需联网，但必须在 `index_local.html` 同级保留完整的 `GeoGebra` 目录。页面仍引用 Google Fonts；断网时浏览器会自动使用系统字体，不影响课件和实验运行。
...
保持文件名不变，刷新课件即可重新读取。若第二个 GGB 使用不同 GeoGebra 版本创建，可能还需要同步修改在线版的 `onlineGGBCodebase` 版本号。
...
2. `assets/1.ggb`、`assets/2.ggb` 路径存在。
...
- 离线版的 GeoGebra 运行资源约占 115 MB，需要单独保存在本地。
